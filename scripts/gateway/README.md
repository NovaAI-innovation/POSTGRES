...
- `core.ps1`: local direct mode script (kept for compatibility).
- `agents/*.ps1`: local direct mode wrappers.
- `.claude/scripts/claude_gateway.ps1`: Claude wrapper that calls central gateway.
- `.codex/scripts/codex_gateway.ps1`: Codex wrapper that calls central gateway.
...
- `AGENT_B_GATEWAY_TOKEN` or `AGENT_B_GATEWAY_API_KEY`
- `AGENT_A_GATEWAY_TOKEN` or `AGENT_A_GATEWAY_API_KEY`
...
- Event-class persistence rules are controlled by each policy file (`claude.json`, `agent-a.json`, `agent-b.json`).
